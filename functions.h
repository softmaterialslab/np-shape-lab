...
inline void SHAKE(INTERFACE& boundary, CONTROL& mdremote, char constraintForm)
{
  ...
  if (constraintForm == 'Q')  // Quadratic slightly better for moderate parameterizations, blows up for extreme ones.
  {
    for(unsigned int k = 0; k < boundary.F.size(); k++)
    {
      for(unsigned int i = 0; i < boundary.F[k].itsV.size(); i++)
        boundary.F[k].itsV[i]->neg_GradConi =  ((2 * (boundary.total_volume - boundary.ref_volume)) ^ boundary.F[k].itsV[i]->neg_GradVi);
    }
  }
  ...
  {
    f0 = f0 + boundary.F[i].itsV[0]->neg_GradConi * (boundary.F[i].itsV[1]->neg_GradConi % boundary.F[i].itsV[2]->neg_GradConi);
    fa1 = fa1 + boundary.F[i].itsV[0]->neg_GradConi * (boundary.F[i].itsV[1]->posvec % boundary.F[i].itsV[2]->neg_GradConi);
    fa2 = fa2 + boundary.F[i].itsV[0]->neg_GradConi * (boundary.F[i].itsV[1]->neg_GradConi % boundary.F[i].itsV[2]->posvec);
    fa3 = fa3 + boundary.F[i].itsV[0]->posvec * (boundary.F[i].itsV[1]->neg_GradConi % boundary.F[i].itsV[2]->neg_GradConi);
    fb1 = fb1 + boundary.F[i].itsV[0]->neg_GradConi * (boundary.F[i].itsV[1]->posvec % boundary.F[i].itsV[2]->posvec);
    fb2 = fb2 + boundary.F[i].itsV[0]->posvec * (boundary.F[i].itsV[1]->posvec % boundary.F[i].itsV[2]->neg_GradConi);
    fb3 = fb3 + boundary.F[i].itsV[0]->posvec * (boundary.F[i].itsV[1]->neg_GradConi % boundary.F[i].itsV[2]->posvec);
  ...
  c = (1.0/mdremote.timestep) * (1.0/mdremote.timestep) * (1.0/mdremote.timestep) * (fc - 6*boundary.ref_volume) / f0;

  // If the constraint is already exactly satisfied (c = 0), bypass the SHAKE:
  if((c == 0)||(f0 == 0))
  { // If scaled volume discrepancy (c = 0), already satisfied (rare). If constraint gradient = 0, already satisfied.
    ...
    cout << "Volume_SHAKE:  Constraint already satisfied: " << c*f0*pow(mdremote.timestep,3)/6  << ".  Bypassing SHAKE." << endl;
...
inline void RATTLE(INTERFACE& boundary, char constraintForm)
{
  ...
  if (constraintForm == 'Q')
  {
    ...
    {
      for(unsigned int i = 0; i < boundary.F[k].itsV.size(); i++)
        boundary.F[k].itsV[i]->neg_GradConi =  ((2.0 * (boundary.total_volume - boundary.ref_volume)) ^ boundary.F[k].itsV[i]->neg_GradVi);
    }
  }
  else // If not quadratic, then linear.  This is the defaut; any non-'Q' flag yields original linear constraint.
  {
  ...
  for (unsigned int i = 0; i < boundary.F.size(); i++)
  {
    ...
    den = den + boundary.F[i].itsV[0]->neg_GradConi * (boundary.F[i].itsV[1]->posvec % boundary.F[i].itsV[2]->posvec)
  ...
  if(num == 0 || den == 0)
  {   // num = 0 if constraint is already satisfied (d/dt(V_tot) = 0) ; den = 0 if constraint gradient (g_i(t)) = 0.
...
inline void SHAKE_for_area(INTERFACE& boundary, CONTROL& mdremote, char constraintForm)
{
  ...
  {
      cout << "SHAKE:  Constraint already satisfied, value: " << boundary.total_volume - boundary.ref_volume  << ".  Bypassing SHAKE, multiplier:  " << x0 << "." << endl;
  }
  else
  {   // Running SHAKE to regulate positions, velocities:
      for (unsigned int i = 0; i < boundary.F.size(); i++)
      {
      ...
      x0 = (-b - sqrt(pow(b,2) - 4*a*c))/(2.0*a); // (Init: {x0 = -25509.2}; typical {x0 <= this}) for qStr = 600.
      x1 = (-b + sqrt(pow(b,2) - 4*a*c))/(2.0*a); // (Init: {x1 = 0, typically x1 ~ -0.1 linear}) for qStr = 600.
      ...
      cout << "SHAKE:  Constraint is NOT satisfied, value: " << boundary.total_area - boundary.ref_area << ".  SHAKING, multiplier:  " << x1 << "." << endl;
  }
...
inline void RATTLE_for_area(INTERFACE& boundary, char constraintForm)
{
  ...
  cout << "RATTLE: Constraint is NOT satisfied, value: " << boundary.total_area - boundary.ref_area << ".  RATTLING, multiplier:  " << mu << "." << endl;
