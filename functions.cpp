...
void initialize_vertex_velocities(vector<VERTEX>& V, vector<THERMOSTAT>& bath)
{
  ...
  for (unsigned int i = 0; i < V.size(); i++)                 // Starting at T_target seems to help stability at times.
...
void interface_movie(int num, vector<VERTEX>& V, INTERFACE& dsphere)
{
  ...
  for (unsigned int i = 0; i < V.size(); i++)
  {
    if (V[i].q > 0)
      ...
      type = "-1";
    outdump << i << "\t" << type << "\t" << V[i].posvec.x << "\t" << V[i].posvec.y << "\t" << V[i].posvec.z << "\t" << V[i].q << "\t" << V[i].itsarea << "\t" << (V[i].q / V[i].itsarea) << endl;
  }
  outdump.close();
  return;
}
