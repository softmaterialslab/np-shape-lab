...
void md_interface(INTERFACE& boundary, vector<THERMOSTAT>& real_bath, CONTROL& cpmdremote, char geomConstraint, char constraintForm)
{
  ...
  for (unsigned int i = 0; i < boundary.V.size(); i++)
  {
    for (unsigned int j = i+1; j < boundary.V.size(); j++)
    {
      VECTOR3D ljforce = lj_vertex_vertex(boundary.V[i], boundary.V[j], boundary.lj_length, boundary.elj);
      VECTOR3D esforce = es_vertex_vertex(boundary.V[i], boundary.V[j], boundary.em, boundary.inv_kappa_out);
  ...
  for (unsigned int i = 0; i < boundary.E.size(); i++)
  ...
  for (unsigned int i = 0; i < boundary.V.size(); i++)
  {   // Re-calculation of the numerical geometric gradients not required as positions haven't changed from above.
  ...
  // Define quantities relevant to simulation visualization:
  int moviestart = 1;	     // starting point of the movie
  int moviefreq = 1000;	   // frequency of shooting the movie
  int offfreq = 2500; 	   // frequency of making off files
  int povfreq = 100000; 	 // frequency of making povray files
  int additional_computation_freq = 50;
  ...
  cout << endl << "\t\tInitial Temperature (prior to MD): " << 2*boundary.kenergy/(real_bath[0].dof*kB) << "\t  &  constraint-induced shift: " << (2*boundary.kenergy/(real_bath[0].dof*kB) - preConTemp) << "." << endl;
  cout << endl << "\t\tInitial Net Energy (prior to MD):  " << initNetEnergy << "\t\t  &  constraint-induced shift: " << (initNetEnergy - preConNetEnergy) << "." << endl << endl;
  ...
  long double fT = pow((1.0/cpmdremote.TAnnealFac),(1.0/cpmdremote.annealDuration));  // Fractional decrement that, when f^(annealDuration), yields (T -> T/10).
  long double fQ = pow((1.0/cpmdremote.QAnnealFac),(1.0/cpmdremote.annealDuration));  // Fractional decrement that, when f^(annealDuration), yields (Q -> Q/2)

  cout << "Initial temperature annealing multiplier: " << fT << " Initial thermostat mass anenealing multiplier: " << fQ << endl << endl;
  ...
  for (int num = 1; num <= cpmdremote.steps; num++)
  {
    ...
    for (unsigned int i = 0; i < boundary.V.size(); i++)
      boundary.V[i].update_real_velocity(cpmdremote.timestep, real_bath[0], expfac_real);	// update particle velocity half time step
    ...
    */
    ...
    for (unsigned int i = 0; i < boundary.V.size(); i++)
      boundary.V[i].compute_neg_VGrads(&boundary/*, constraintForm*/);
      // Initialize the net forces on all vertices:
    for(unsigned int i=0; i < boundary.V.size(); i++)
      boundary.V[i].forvec = 0;
    ...
    if (num <= 1000 || num % additional_computation_freq == 0)
    {
    ...
    list_energy << num << setw(15) << boundary.kenergy << setw(15) << boundary.penergy << setw(15) << boundary.energy << setw(15) << boundary.energy 
    + real_bath_ke + real_bath_pe << setw(15) << real_bath_ke << setw(15) << real_bath_pe << endl;
    
      // Print the initial and current net energy, along with the drift (their ratio):
    cout << "(" << num << ")" << "\tInitial Net Energy: " << initNetEnergy << "\t & \t Current Net Energy: " << (boundary.energy + bath_kinetic_energy(real_bath) + bath_potential_energy(real_bath)) << "\t & \t Drift: " << ((boundary.energy + bath_kinetic_energy(real_bath) + bath_potential_energy(real_bath))/initNetEnergy) << "." << endl;
    ...
    */
    ...
    if (cpmdremote.anneal == 'y' && num > cpmdremote.annealDuration && ((num % cpmdremote.annealfreq == 0) || (num % cpmdremote.annealfreq < cpmdremote.annealDuration)) && real_bath[0].T >= 0.000000000000001)
    {
      ...
      { // Decide which fractional reduction for {T, Q} to use:
        ...
        else if((6*cpmdremote.annealfreq) <= num < (7*cpmdremote.annealfreq)) // If in 8th, decrement temperature by 8.0x.
        ...
        fT = pow((1.0/cpmdremote.TAnnealFac),(1.0/cpmdremote.annealDuration));  // (fT != TAnnealFac if annealDuration != 1)
        fQ = pow((1.0/cpmdremote.QAnnealFac),(1.0/cpmdremote.annealDuration));

        cout << "Beginning annealing for " << cpmdremote.annealDuration << " steps decrementing by a factor of fT = " << fT << " and fQ = " << fQ << "." << endl;
      }
  ...
  if(1.05 < ((boundary.energy + bath_kinetic_energy(real_bath) + bath_potential_energy(real_bath))/initNetEnergy)) abort();
  //  Also abort if annealing hasn't yet begun, but the global energy has drifted downward by more than 5%:
  else if((num < cpmdremote.annealfreq) && (((boundary.energy + bath_kinetic_energy(real_bath) + bath_potential_energy(real_bath))/initNetEnergy) < 0.95)) abort();
  }
}
