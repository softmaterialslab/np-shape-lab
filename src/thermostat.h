...
public:
    ...
    double xi;            // thermostat variable                                          // NB:  Thermostat "velocity"
    ...
    // make a thermostat
    THERMOSTAT(double mass = 0.0, double temperature = 0.0, int dof = 0, double xi = 0.0, double eta = 0.0,
               double hold = 0.0) : Q(mass), T(temperature), dof(dof), xi(xi), eta(eta), hold(hold) {
    }
